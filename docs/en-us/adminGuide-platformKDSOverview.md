...
For this example, fired items are sent to both the expediter KDS device and the appropriate prep station KDS devices. When all items are marked as fulfilled at the prep station, the ticket turns green on the expediter KDS device, indicating that all items are ready. Once the items are physically at the expediter station, the ticket is marked as fulfilled. If you have notifications configured to appear when tickets are fulfilled, the Toast POS device that placed the order initially receives a notification. In this case, the prep stations are configured to [send items and tickets to the expediter once completed](adminKitchenDiningRoomReference.html#configSendToExpediter), and the kitchen uses a [single-level expediter](adminKitchenDiningRoomReference.html#configTwoLevelFulfillment). You can also configure a prep station to not send fulfilled items and tickets to the expediter. For example, the Bar prep station is not in the kitchen, so it does not need to send fulfilled items to the expediter KDS device located in the kitchen.
...
For this example, fired items are sent to the appropriate prep station KDS devices. When individual items are marked as fulfilled on the prep station screen, a green check appears next to the item. Once all items on a ticket are marked as fulfilled, the ticket disappears. If the prep station is configured to [not send items and tickets to the expediter once completed](adminKitchenDiningRoomReference.html#configSendToExpediter) and you have notifications configured to appear when tickets are fulfilled, the Toast POS device that placed the order initially receives a notification.
...
- Change ticket size: This option changes the ticket size and layout used by grid view for KDS devices. For more information, see [Ticket sizing and layout](platformKDSOverview.html#platformGridKDSGridLayout).
...
- [Grid layout](platformKDSOverview.html#platformGridKDSGridLayout): Provides a grid layout for kitchen tickets with a maximum number of tickets on a screen.


- [Preview tickets](platformKDSOverview.html#platformGridKDSPreviewTickets): Provides kitchen employees a preview of upcoming tickets as items are added to an order.


- [Pagination](platformKDSOverview.html#platformGridKDSPagination): Indicates how many pages of tickets exist and can be tapped to quickly move to a specific page.


- [Payment status](platformKDSOverview.html#platformGridKDSPaymentStatus): Displays whether the order was paid or unpaid.


- [Dark mode](platformKDSOverview.html#platformGridKDSDarkMode): A color scheme for the screen that is more optimal for low light or prolonged screen use.


- [Food runner fulfillment](platformKDSOverview.html#platformKitchenKDSFoodRunnerFulfillment): Individual items can be marked as fulfilled on expediter KDS devices. If already fulfilled at the previous level (prep station KDS devices for one expediter and the first level expediter for two-level fulfillment), fulfillment of an item on an expediter ticket uses a double check mark.
...
> **Important**
> 
> Grid view is only compatible with the local data synchronization method. This local sync method allows for offline mode using local sync. For more information on how local sync works, see [Offline mode with local sync](platformOfflineModeLocalSync.html).
...
- Change ticket size: Opens the Change ticket size dialog. This dialog changes the size and maximum number of tickets shown on the screen for the [grid layout](platformKDSOverview.html#platformGridKDSGridLayout). This is specific to grid view.
...
The bottom of the screen is for navigation and page indicators. For more information, see [Pagination](platformKDSOverview.html#platformGridKDSPagination).
...
> **Note**
> 
> You must use grid view to configure ticket size. For more information on how to configure grid view, see [Setting up grid view](platformKDSOverview.html#platformGridKDSSetUp).
...
- You set the [Fulfillment Method](adminKitchenDiningRoomReference.html#configFulfillmentMethod) setting to Fulfill at each station independently.
...
If you set the [Fulfillment Method](adminKitchenDiningRoomReference.html#configFulfillmentMethod) setting to Fulfill at each station independently, a partial fulfillment indicator appears on expediter KDS tickets for items that have been partially fulfilled.



> **Note**
> 
> The prep station assigned to an item must [send to expediter](adminKitchenDiningRoomReference.html#configSendToExpediter) in order to trigger the partial fulfillment on the expediter KDS device tickets.
...
> **Note**
> 
> To use food runner fulfillment, the [Fulfill Items](adminKitchenDiningRoomReference.html#configFulfillItems) setting must be set to Enable individual item fulfillment.
...
- If you use [two-level fulfillment](adminKitchenDiningRoomReference.html#configTwoLevelFulfillment), the highest level of fulfillment is at the second-level expediter KDS device. The green double check mark appears when the item is fulfilled at the second-level expediter KDS device.
...
- Do not disconnect your local hub device from your router. The local hub device is required for offline mode with local sync. For more information about the local hub device, see [Local hub devices](platformOfflineModeLocalSync.html#platformOfflineModeLocalSyncLocalHubDevice).
...
| Setting | Behavior | 
| --- | --- |
| Text Size | For grid view, the text size is associated with the ticket size. For more information, see [Procedure 10.30, “To configure the ticket size”](platformKDSOverview.html#platformGridKDSToConfigureSize). | 
