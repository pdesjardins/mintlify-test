...
(1) Send a POSTrequest to the /era/v1/metricsendpoint of the analytics API.
...
(4) The list of restaurant GUIDs from the management group to exclude from the aggregated sales reporting data. In this example, restaurants are included with the restaurantIdsvalue, so all other restaurants are excluded automatically.
...
(1) Send a POSTrequest to the /era/v1/metrics/{timeRange}endpoint of the analytics API. The {timeRange}in this example is week.
...
(2) The end date of the time range for the aggregated sales reporting data, in YYYYMMDDformat. The number of days in the time range must be seven or fewer because the request is for a weektime range.
