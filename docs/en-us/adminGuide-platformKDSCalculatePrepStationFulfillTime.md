...
In the case of [individual item fulfillment](adminKitchenDiningRoomReference.html#configFulfillItems), if two items are on the same ticket and one item is fulfilled before the other, each item has a separate prep station fulfillment time.

In the case of [assembly lines](platformKDSOverview.html#platformGridKDSAssemblyLines), if [individual item fulfillment](adminKitchenDiningRoomReference.html#configFulfillItems) is enabled, individual items can be fulfilled on a ticket before the entire ticket is fulfilled. At this point, the fulfilled item or items become a new ticket at the next prep station in the assembly line sequence. The prep station fulfillment time for the next prep station is calculated based on when the new ticket arrived and got fulfilled. The prep station fulfillment time for the previous prep station is calculated based on when the rest of the ticket is fulfilled.
