...
: To receive notifications about planned and unplanned outages,
          sign up for API status notifications at [http://status-dev.toasttab.com](http://status-dev.toasttab.com).



****Release notes****
: Sign up for Toast’s [API release note
          distribution list](apiDevGuide-apiUpdatesEmailDistributionList).
...
- If your integration retrieves information such as [orders](apiDevGuide-apiOrdersGetDetailedInfoAboutMultipleOrders)
        or [time entries](apiDevGuide-apiGettingTimeEntriesForEmployees)
...
Scheduled maintenance windows are announced in advance on [http://status-dev.toasttab.com](http://status-dev.toasttab.com).
...
- To minimize your integration's downtime, monitor the
              status notifications on [http://status-dev.toasttab.com](http://status-dev.toasttab.com).
...
- If your authentication token expires during the
              ...
              authentication, see [Getting an authentication token](apiDevGuide-authentication#getting-authentication-token) and [Refreshing authentication tokens](apiDevGuide-apiAuthTokenRefresh).
...
You should include a unique external identifier with each order you
    ...
    more information about using external identifiers, see [External identifiers](apiDevGuide-portalToastIdentifiers#apiExternalIdentifiers).
...
- Prevent guests from [scheduling future
          orders](apiDevGuide-orders_api_future_orders) during the planned maintenance window.
...
First, evaluate whether the order submission truly succeeded by
      ...
      an order, see [Getting detailed information about one order](apiDevGuide-apiOrdersGetDetailedInfoAboutOneOrder).
