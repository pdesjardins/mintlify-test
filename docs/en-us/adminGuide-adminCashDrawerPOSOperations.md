...
To review details of the Cash In entry, select the cash entry on the cash drawer activity panel. A dialog appears with details about the Cash Inentry.
...
- Toast POS: You can complete a payout from the Pay out screen on a Toast POS device. On the Toast POS home screen, choose Cash Management, and then select Pay out. Paying out from the Pay outscreen allows you to make payouts from any cash drawer that you have permissions for.
...
5. This navigates you back to the Remove cash screen. On the Remove cashscreen, the payout reason is listed below the Details heading on the cash drawer activity panel. Use the numerical keypad to enter the cash amount. To add a comment, select the + Add comment button. This opens a dialog where you can enter a comment.
...
> **Note**
> 
> Avoid using the Tip Out action on the Cash Drawers screen if employees must complete shift review. Tip outs made on the Cash Drawersscreen can lead to incorrect tip out amounts in shift review.
...
This navigates you back to the Remove cash screen. On the Remove cashscreen, the employee name is listed below the Details heading on the cash drawer activity panel.
...
4. Select the Save button. The new no sale reason appears on the No Sale Reasonstable on the Cash drawer activityscreen.
...
You can now choose to create a deposit for the cash in the cash drawer. For more information, see [Cash deposits](adminCashDeposits.html).
...
- This opens the Count billsscreen. On the Count bills screen, use the bill and coin calculator to count the amount of cash collected.
...
3. Complete the close drawer workflow. For more information, see the section on how to [close an open cash drawer](adminCashDrawerPOSOperations.html#adminCashDrawerClose).
...
A new cash drawer is opened if you choose to count the cash drawer’s cash later. The new cash drawer appears under the Open tab on the Cash Drawersscreen.
...
- Automatic: Toast automatically uses the amount in the Starting Cash Drawer Balancefield when a new drawer is created.
...
1. On the Cash Drawers screen, navigate to the Closed tab and select the closed cash drawer to open the Cash drawer activityscreen.
...
2. Select the Edit starting balance button to open the Adjust cash drawer starting balancedialog.
