...
> **Note**
> 
> Some jobs may require you to clock in to your shift in order to complete shift review. For more information, see [Requiring shift review](docs/en-us/adminGuide-platformConfiguringShiftReview#platformRequiringShiftReview).


If you are required to clock in to complete your shift review, but have not clocked in yet, an error message appears on the Shift Review screen. The error message displays: `Please clock in.`
...
> Employees who must declare cash tips must have the Tipped? option checked for their assigned job. For more information, see [UI options settings](docs/en-us/adminGuide-adminUiOptionsReference).
...
> **Note**
> 
> To allow employees to cash out their credit card tips without having to enter a manager passcode, they must be assigned the 1.13 No Sale permission. This permission allows employees to make a No Sale entry and open a cash drawer without having to complete a transaction. For more information about permissions, see [Access permissions reference](docs/en-us/adminGuide-adminPermissions#adminAddlPosAccessPermissions).
...
The next step in shift review is closing any cash drawer(s) locked to you. This is an optional step that can be configured on the Toast POS device. For more information on how to lock down a cash drawer, see [Lock down a cash drawer](docs/en-us/adminGuide-adminCashDrawerLockdown).
