---
title: "Bump bar usage guidelines"
id: platformKitchenBumpBarGuidelines
type: section
...
previousSectionFile: adminGuide-platformKitchenUsingBumpBars.md
previousSectionTitle: "Using the bump bar"
...
- **Do not** disconnect the bump bar and then switch users. This may result in an error that causes the bump bar to be unable to select tickets once reconnected if there are multiple pages of tickets.



When using the bump bar, consider the following limitations:

- You need to use the touchscreen to navigate any part of the Toast POS device that is not the passcode screen or the KDS mode using grid view.


- You need to use the touchscreen to access the overflow menu (the ⋮ icon) on the KDS device.


- You need to use the touchscreen to access the average fulfillment timer feature on the KDS device. For more information, see [Average fulfillment timer](docs/en-us/adminGuide-platformKDSOverview#platformKDSGridAvgFulfillmentTimer).
...
- Changes conflicts with other devices
...
- You need to use the touchscreen to access and change any settings.
