...
- [adminGuide#platformMarkingKDSOrdersAsOrderReady]
...
- `IN_PREPARATION`: The order is being actively worked on. This status corresponds to the [adminGuide#platformActiveOrders]status in Orders Hub.


- `READY_FOR_PICKUP`: The order is ready for guest pickup. This status corresponds to the [adminGuide#platformOrderReadyOrders]status in Orders Hub.


- `CLOSED`: The order is paid for and completed. This status corresponds to the [adminGuide#platformCompletedOrders]status in Orders Hub. If a closed order is reopened, this moves the order back to the `IN_PREPARATION`status.
...
> **Note**
> 
> The Kitchen Display System (KDS) must be configured and enabled to automatically mark KDS-fulfilled orders as Order Readyin Orders Hub. For more information, see [adminGuide#platformMarkingKDSOrdersAsOrderReady].


When an order is marked as Order Readyin Orders Hub or is fulfilled by the Kitchen Display System (KDS), this triggers the `guest order fulfillment status`webhook to send a message. The message is sent to the third-party online ordering channel that the order was placed through. For example, if a guest places an order through DoorDash, the DoorDash driver is sent a message notifying them that the guest’s order is now ready for pickup.
...
- If the order has been fulfilled, the order will be removed from Sales summary report and shown as Voidedin the Orders report.
