...
> **Note**
> 
> The [Course Pacing](adminKitchenDiningRoomReference.html#configCoursePacing) setting must have the Enable course pacing checkbox selected to access the [Sending Courses](adminKitchenDiningRoomReference.html#configSendingCourses), [Firing Courses](adminKitchenDiningRoomReference.html#configFiringCourses), and [Automated Firing](adminKitchenDiningRoomReference.html#configAutomatedFiring) settings.
...
The following example shows the expediter tickets for an order with two drinks in a Drinks course, two appetizers in an Appetizers course, and two entrees in an Entrees course (the prep station tickets for this order are shown [above](adminInterpretingTicketTimes.html#adminFireTimesPrepStationTickets)). An employee sent the Drinks course to the kitchen and held the Appetizers and Entrees courses. Later, the employee fired the Appetizers course. The ticket time and fire time for the Drinks course ticket are the same.
