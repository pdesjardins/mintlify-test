...
4. With the Transaction Limit setting, you can optionally set a dollar amount limit for credit card transactions taken while in offline mode. Note that this threshold is not applied to employees who have the [Offline/Background Credit Card Processing](docs/en-us/adminGuide-adminPermissions#permissionOfflineCCProcessing) permission. A payment larger than the configured threshold requires manager approval.
