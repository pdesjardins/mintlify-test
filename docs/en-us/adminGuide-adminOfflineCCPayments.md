...
You can set the **Card authorization and payment disruption** option to control whether your restaurant can still process card transactions after losing its connectivity. As part of the card payment workflow, card authorization requests are processed in real time or in the background, depending on the configured **Card authorization and payment disruption**setting.
