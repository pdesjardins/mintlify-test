...
Discounts are often configured with [required items](adminDiscountRequiredItemsAbout.html). The discount can only be applied when the guest purchases a specific item or combination of items.

You can also configure a discount with an [availability range of dates, days, and times](adminDiscountAvailability.html). Even if a check includes the required items, if the current date is not within the availability range, the discount cannot be applied.
...
Otherwise, a restaurant employee selects the applicable discount from the Toast POS app. The discount configuration includes [how to display it on the Toast POS device](adminDiscountsPosDisplay.html).
...
: When a discount is applied to a check, then the discount configuration determines the [effect on the check prices](adminDiscountPricing.html).
