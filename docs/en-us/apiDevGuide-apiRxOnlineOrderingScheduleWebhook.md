...
The restaurant online ordering schedule webhook messages follow the [standard message data schema](docs/en-us/apiDevGuide-apiMessageDataSchema). When a message is published to your webhook endpoint for the `order_schedule` event category, the `eventCategory` value is set to `ordering_schedule`and the `eventType` is set to `ordering_schedule_updated`.

## Configuring online ordering schedules 

You can configure your online ordering schedule and overrides in Toast Web. Choose Takeout & delivery &gt; Online ordering hours to open the Online ordering hourssection. For more information, see [Online ordering hours overview](docs/en-us/adminGuide-adminOnlineOrderingScheduleOverview).
