...
Here are the current rate limits for Toast APIs. These rate limits are described in more detail in [How rate limiting works](docs/en-us/apiDevGuide-apiRateLimiting#apiRateLimitingOverview).
...
> **Note**
> 
> An [API account](docs/en-us/apiDevGuide-apiClientAccounts#apitypesOfToastApiAccounts) that integrates with many restaurant locations may have additional, account-specific rate limits. These account rate limits track the total number of requests the client integration makes to certain APIs or endpoints *across all restaurant locations*.
> Toast support reaches out directly to integration providers to discuss the need for an account rate limit before putting the rate limit in place. See [Account rate limits](docs/en-us/apiDevGuide-apiRateLimiting#apiAccountRateLimits) for more information.
...
For more information, see [Rate limiting header fields](docs/en-us/apiDevGuide-apiRateLimiting#apiRateLimitingHeaderFieldsV2).

### Rate limits use fixed time slices

Toast APIs use fixed time slices when they calculate whether the [individual limits within a rate limit](docs/en-us/apiDevGuide-apiRateLimiting#apiRateLimitesAreACollectionOfLimits) were reached.
...
When this header field contains `API` or `ENDPOINT`, it may also contain `ACCOUNT` to indicate that an account rate limit on an API or endpoint is close to being exceeded. When `X-Toast-RateLimit-By` contains more than one value, the values are comma-separated. See [Account rate limits](docs/en-us/apiDevGuide-apiRateLimiting#apiAccountRateLimits) for more information.
...
> **Note**
> 
> Rate limiting header fields are not included in authentication API responses. See [Authentication rate limit](docs/en-us/apiDevGuide-apiAuthenticationRateLimit) for more information.
