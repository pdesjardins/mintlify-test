...
- The unique `guid` assigned to each menu entity. You can see examples of that in [Order details based on the order dining option](docs/en-us/apiDevGuide-apiOrderTypeDetails) and [Specifying modifiers and instructions for menu item selections](docs/en-us/apiDevGuide-apiSpecifyingModifiersAndInstructions).


- The `multilocationId` for each menu entity. This is the recommended option, especially for restaurants that use the multi-location management module (previously known as the "[enterprise module](docs/en-us/adminGuide-sharingMenusAndOtherInformationAmongRestaurants)").



For information about `guid` and `multilocationId` values and using them in API requests, see [Toast identifiers](docs/en-us/apiDevGuide-portalToastIdentifiers).
