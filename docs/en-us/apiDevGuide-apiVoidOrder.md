...
If an order is placed and needs to be canceled, you can use the
  `/orders/v2/orders/<em>{orderGuid}</em>/void`
...
When you void an order, you must also void the order's payment at the
  ...
  the order with the `/orders/<em>{guid}</em>`
  and `/ordersBulk` endpoints of the orders API. For more
  information about retrieving an order, see [Getting detailed information about one order](apiDevGuide-apiOrdersGetDetailedInfoAboutOneOrder).
...
- Your API client must have the `orders.channel:void`
        scope. For more information about scopes, see [Scopes](apiDevGuide-apiScopes).


- The order was placed using an Other payment
        option and not with a cash or card payment. For more
        information about other payment options, see [Other payment workflows](adminGuide-platformPwfTenders#platformPwfOther).
...
- The order is not restricted. For more information about
        restricted orders, see [Working with restricted orders](adminGuide-adminViewingRestrictedOrders).


- You are using the same `clientId` in your [authentication](apiDevGuide-authentication) that was used to
...
To void an order, you send a `POST` request
    ...
    [prerequisites](apiDevGuide-apiVoidOrder#apiVoidOrderPrereq).
...
> **Note**
> 
> Each `voidAll` value must be set to
          `true`.



2. Send a `POST` request to the
        `/orders/v2/orders/<em>{order GUID}</em>/void` endpoint of the orders API.
...
You can void an order that has discounts if the order meets all the
    [prerequisites](apiDevGuide-apiVoidOrder#apiVoidOrderPrereq). When you void an
    order with discounts, you use the same [API request](apiDevGuide-apiVoidOrder#apiVoidOrderToVoidAnOrder) that you use to
...
**Example 2.1. Example orders API response of a voided order with
      discounts**
...
- Other payment types.: The
        ...
        Other payment types, see [Other payment workflows](adminGuide-platformPwfTenders#platformPwfOther).
