...
You can disable inherited modifier groups for an individual item within a menu group or subgroup, by setting the item's Inherit modifier groups? setting to No. If you set a menu item's Inherit modifier groups? setting to No, it will not inherit *any* of the modifier groups assigned to *any* of its parent menu groups. For more information, see [Inheriting modifier groups from a parent menu group](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminInheritingMenuGroupsFromParentMenuGroup).



> **Note**
> 
> The remainder of this section uses the term *menu group* to refer to both menu groups and subgroups. Menu groups and subgroups have the same configuration settings and are effectively the same except for their location in the menu hierarchy. When there is a need to differentiate, the documentation does so. For more information, see [Comparing menu groups and subgroups](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminComparingMenuGroupsAndSubgroups).





- To reduce the number of modifier groups you have to manage, [add existing modifier groups](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminAddingAnExistingModifierGroupToAMenuGroupOrItem) to menu groups or items whenever possible instead of creating new ones. For example, add the same Salad Dressing modifier group to both the Dinner Salad and Side Salad menu items.


- If you remove a modifier group from a menu group, it is removed for all of the menu items in that menu group. For more information, see [Removing a modifier group](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminRemovingAModifierGroup).
...
You can disable inherited modifier groups for an individual item within a menu group or subgroup, by setting the item's Inherit modifier groups? setting to No. If you set a menu item's Inherit modifier groups? option to No, it will not inherit *any* of the modifier groups assigned to *any* of its parent menu groups. For more information, see [Inheriting modifier groups from a parent menu group](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminInheritingMenuGroupsFromParentMenuGroup).
...
1. On the details page for the [menu group](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuGroup), [subgroup](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditASubgroup), or [item](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuItem)that you want to add a modifier group to, select Modifier groups.
...
4. In the Modifier group details section, if you use Toast POS devices, you can [customize the POS button name and color](docs/en-us/adminGuide-adminAssigningPOSButtonNicknameColorinMenuBuilder).


5. In the Pricing section, [specify how the modifiers in this group are priced](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminSpecifyingModifierGroupPricing).


6. In the Modifiers section, [add modifiers to your modifier group](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminAddingModifierstoModifierGroup).


7. In the Modifier group settings section, [choose whether selecting a modifier from this group is required or optional, and, if it's optional, whether to prompt the server](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminSpecifyingModifierGroupRequiredOptional).


8. In the Modifier group settings section, [specify whether you want guests to be able to order more than one modifier from this group, the minimum and maximum number of modifiers to choose from, and whether a modifier can be chosen more than once](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminAllowingCustomerstoChooseOneorMoreModifiersfromModGroup).


9. In the Channel visibility section, [select the ordering channels where this modifier group is visible](docs/en-us/adminGuide-adminSpecifyingOrderingChannelVisibility). A modifier group is available for ordering on the channels where it is visible.
...
1. On the details page for the [menu group](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuGroup), [subgroup](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditASubgroup), or [item](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuItem) that you want to add an existing modifier group to, select Modifier groups.
...
The Modifier groups section of the [menu item details](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuItem) page has an Inherit modifier groups? setting. By default, this setting is set to Yes, so that the menu item inherits modifier groups from its parent menu groups.
...
For more information on inheritance, see [Understanding inheritance](docs/en-us/adminGuide-platformUnderstandingInheritance).
...
1. On the details page for the [menu group](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuGroup), [subgroup](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditASubgroup), or [item](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuItem)that contains the modifier group that you want to edit, select Modifier groups.
...
You use the Pricing section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup) to specify how the modifiers in a group are priced:
...
- To reduce the number of modifiers you have to manage, [add existing modifiers](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#platformAddingAnExistingModifierToAModifierGroup) to modifier groups whenever possible instead of creating new ones. For example, a Breakfast Sides modifier group and a Burger Toppings modifier group could share a Bacon modifier.
...
1. In the Modifiers section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), enter the name of the first modifier in the first row.
...
3. If your modifiers are [priced individually](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminSpecifyingModifierGroupPricing), the Modifiers table has a Price column where you can enter a price for each modifier you create.
...
1. In the Modifiers section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), select the arrow next to the + Add button and then choose Add existing from the menu.
...
7. If you are ready for guests and employees to see these menu changes, select Publish all changes, then select Publish again in the Confirm Publish dialog. For more information, see [Understanding when to publish your menu](docs/en-us/adminGuide-adminUnderstandingWhenToPublishYourMenu).
...
1. In the Modifiers section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), select a modifier's reorder icon and drag the row to its new location.
...
4. If you are ready for guests and employees to see these menu changes, select Publish all changes, then select Publish again in the Confirm Publish dialog. For more information, see [Understanding when to publish your menu](docs/en-us/adminGuide-adminUnderstandingWhenToPublishYourMenu).



### Editing a modifier's details

The Modifiers table displays a modifier's name and price (the Price column only appears if the modifiers in the group are [priced individually](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminSpecifyingModifierGroupPricing)). To set other details for a modifier, such as button color and calories, you open its Edit modifier dialog.

**Procedure 8.25. To set other details for a modifier**

1. In the Modifiers section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), select a modifier's edit icon.
...
1. In the Modifiers section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), select a modifier's remove icon.
...
4. If you are ready for guests and employees to see these menu changes, select Publish all changes, then select Publish again in the Confirm Publish dialog. See [Understanding when to publish your menu](docs/en-us/adminGuide-adminUnderstandingWhenToPublishYourMenu) for more information.



## Specifying if a modifier group is required or optional

In the Modifier group settings section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), you can choose whether selecting a modifier from a group is required or optional:
...
In the Modifier group settings section of a [modifier group's details page](docs/en-us/adminGuide-adminAddingModifierGroupsAndModifiers#adminEditingAModifierGroup), you can specify whether guests can order more than one modifier from a modifier group.
...
1. On the details page for the [menu group](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuGroup), [subgroup](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditASubgroup), or [item](docs/en-us/adminGuide-adminCreatingYourMenuHierarchy#adminToEditAMenuItem)that contains the modifier group that you want to remove, select Modifier groups.
