---
title: "Using the kitchen display system"
id: adminKitchenDisplaySystemOmitChunkFromSearchIndex
type: section
...
previousSectionFile: adminGuide-platformKdsStartQueue.md
previousSectionTitle: "Using start queue"
...
For this example, fired items are sent to both the expediter KDS device and the appropriate prep station KDS devices. When all items are marked as fulfilled at the prep station, the ticket turns green on the expediter KDS device, indicating that all items are ready. Once the items are physically at the expediter station, the ticket is marked as fulfilled. If you have notifications configured to appear when tickets are fulfilled, the Toast POS device that placed the order initially receives a notification. In this case, the prep stations are configured to [send items and tickets to the expediter once completed](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configSendToExpediter), and the kitchen uses a [single-level expediter](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configTwoLevelFulfillment). You can also configure a prep station to not send fulfilled items and tickets to the expediter. For example, the Bar prep station is not in the kitchen, so it does not need to send fulfilled items to the expediter KDS device located in the kitchen.
...
For this example, fired items are sent to the appropriate prep station KDS devices. When individual items are marked as fulfilled on the prep station screen, a green check appears next to the item. Once all items on a ticket are marked as fulfilled, the ticket disappears. If the prep station is configured to [not send items and tickets to the expediter once completed](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configSendToExpediter) and you have notifications configured to appear when tickets are fulfilled, the Toast POS device that placed the order initially receives a notification.
...
At the top-left of the screen, the arrow icon returns you to the Toast home page. On the top-right, there are options to show the [all day view](adminReportingTotalsKDS.html#adminAllDayView), show [recently fulfilled tickets](adminRedisplayingTickets.html), and [recalling the last fulfilled ticket](adminRedisplayingTickets.html#adminRecallTicket). If there is not enough space at the top to display these options, they can appear in the overflow menu. The overflow menu, represented by three vertically stacked dots, provides additional actions and shortcuts.
...
- Change ticket size: This option changes the ticket size and layout used by grid view for KDS devices. For more information, see [Ticket sizing and layout](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSGridLayout).
...
At the bottom of the screen, there are production items, if configured. For more information about production items, see [About production items](platformKitchenOperationsAndFulfillmentOmitChunkFromSearchIndex.html#adminAboutProdItems).
...
- [Grid layout](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSGridLayout): Provides a grid layout for kitchen tickets with a maximum number of tickets on a screen.


- [Preview tickets](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSPreviewTickets): Provides kitchen employees a preview of upcoming tickets as items are added to an order.


- [Pagination](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSPagination): Indicates how many pages of tickets exist and can be tapped to quickly move to a specific page.


- [Payment status](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSPaymentStatus): Displays whether the order was paid or unpaid.


- [Dark mode](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSDarkMode): A color scheme for the screen that is more optimal for low light or prolonged screen use.


- [Food runner fulfillment](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformKitchenKDSFoodRunnerFulfillment): Individual items can be marked as fulfilled on expediter KDS devices. If already fulfilled at the previous level (prep station KDS devices for one expediter and the first level expediter for two-level fulfillment), fulfillment of an item on an expediter ticket uses a double check mark.
...
> **Important**
> 
> Grid view is only compatible with the local data synchronization method. This local sync method allows for offline mode using local sync. For more information on how local sync works, see [Offline mode with local sync](devPortalPlatformGuideNetworkConnectionsOmitChunkFromSearchIndex.html#platformOfflineModeLocalSync).
...
At the top-left of the screen, the arrow icon returns you to the Toast POS home screen. On the other side, there are options to show the [recently fulfilled](adminRedisplayingTickets.html), and [recall the last fulfilled ticket](adminRedisplayingTickets.html#adminRecallTicket).
...
- Change ticket size: Opens the Change ticket size dialog. This dialog changes the size and maximum number of tickets shown on the screen for the [grid layout](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSGridLayout). This is specific to grid view.
...
The bottom of the screen is for navigation and page indicators. For more information, see [Pagination](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSPagination).
...
> **Note**
> 
> You must use grid view to configure ticket size. For more information on how to configure grid view, see [Setting up grid view](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSSetUp).
...
If you set the [Fulfillment Method](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configFulfillmentMethod) setting to Fulfill at each station independently, a partial fulfillment indicator appears on expediter KDS tickets for items that have been partially fulfilled.



> **Note**
> 
> The prep station assigned to an item must [send to expediter](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configSendToExpediter) in order to trigger the partial fulfillment on the expediter KDS device tickets.
...
> **Note**
> 
> To use food runner fulfillment, the [Fulfill Items](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configFulfillItems) setting must be set to Enable individual item fulfillment.
...
- If you use [two-level fulfillment](platformKitchenConfigReferenceOmitChunkFromSearchIndex.html#configTwoLevelFulfillment), the highest level of fulfillment is at the second-level expediter KDS device. The green double check mark appears when the item is fulfilled at the second-level expediter KDS device.
...
- Do not disconnect your local hub device from your router. The local hub device is required for offline mode with local sync. For more information about the local hub device, see [Local hub devices](devPortalPlatformGuideNetworkConnectionsOmitChunkFromSearchIndex.html#platformOfflineModeLocalSyncLocalHubDevice).
...
| Setting | Behavior | 
| --- | --- |
| Text Size | For grid view, the text size is associated with the ticket size. For more information, see [Procedure 10.30, “To configure the ticket size”](adminKitchenDisplaySystemOmitChunkFromSearchIndex.html#platformGridKDSToConfigureSize). | 
