...
Menus webhook messages follow the [standard message data schema](apiDevGuide-apiMessageDataSchema). When a message is published to your webhook endpoint for the `menus`event category, the `eventCategory` value is set to `menus`. Currently, `menus_updated` is the only `eventType` for the `menus` event category. For details on the payload for this event type, see [menus_updated](apiDevGuide-apiMenusWebhook#apiMenusUpdated).
