...
> You can include restaurant GUIDs for either
        ...
        not for both. Listing restaurant GUIDs for both results in a 400 error
        or blank guest reporting data. `restaurantIds` and
        `excludedRestaurantIds` are contradictory values that
...
(1) Send a POST request to the
            /era/v1/guest/payments/{timeRange} endpoint of the
...
(4) Include details about the requested guest reporting data in
...
(1) The list of restaurant GUIDs from the management group to
            include in the guest reporting data. Restaurant GUIDs not listed
            are excluded.
...
(3) The start date of the time range for the guest reporting
            data, in YYYYMMDD format.

(4) The end date of the time range for the guest reporting data,
            in YYYYMMDD format.
...
(1) The GUID for the guest reporting data request, also called
...
For an example about how to retrieve the guest reporting data,
        see [Retrieving the guest reporting data](apiDevGuide-apiAnalyticsGuestDataRetrieveData).
