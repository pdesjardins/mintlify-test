...
- Bulk discounts. These are fixed currency check-level discounts and fixed percent check-level discounts that are configured with at least one required item, and are configured as [bulk discounts](adminBulkDiscounts.html).
