...
In the **Tip** field, enter the amount of the tip to refund. Note that the **Original Amount**value for the tip does not include [gratuity service charges](adminGuide-adminServiceChargeType). These charges are listed on the previous screen.
...
You cannot use the **Tip only** option to refund a gratuity service charge. To refund a service charge, use the **By item(s)** or **Entire check**option.
