...
In your implementation of the Toast loyalty integration API, you create a single HTTPS REST endpoint, named in any way you choose. For example: `https://<em>myapi.mycompany.com</em>/<em>myendpointname</em>`. The Toast platform sends requests to that single endpoint (host name and path) for all loyalty program transactions. You can identify the requests for specific transaction types using the *`Toast-Transaction-Type`* HTTP header parameter for each request that you receive.
