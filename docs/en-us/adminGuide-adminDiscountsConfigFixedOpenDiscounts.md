...
On the discount configuration page, when you select either **Fixed $ Off** or **Fixed % Off**as the discount type, a **Value** field is displayed.
...
The available values for **Applies to**are:
...
- **Allow with other discounts**enabled. Discounts that have excluded items must be exclusive.
