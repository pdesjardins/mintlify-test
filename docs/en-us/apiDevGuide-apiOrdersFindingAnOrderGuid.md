...
- If you have created an order using the orders API, the response
        ...
        information about creating orders see [Creating orders](apiDevGuide-apiCreatingOrders).
...
- Send a `GET` request to the
        ...
        order GUID. See [Getting detailed information about multiple orders](apiDevGuide-apiOrdersGetDetailedInfoAboutMultipleOrders) for more
...
- If you know the order GUID, you can send a `GET`
        ...
        details, which include the check GUID. See [Getting detailed information about one order](apiDevGuide-apiOrdersGetDetailedInfoAboutOneOrder).
