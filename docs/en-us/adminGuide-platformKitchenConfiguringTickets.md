...
4. Expand the advanced properties table to find the modifier you want to edit, select its name, then select Edit Item to see the classic details page for the modifier's [item reference](docs/en-us/adminGuide-adminPricingModifierOptions#adminUnderstandingAModifierOptionsItemReference).


5. Next to the KDS Color, select the square tile. The [color palette](docs/en-us/adminGuide-platformKitchenConfiguringTickets#adminKdsColorPalette)opens.
...
You can choose to sort items by assigned seat numbers, assigned sort order, or by how the items are entered on the order screen. You can use the [Item Sorting Priority](docs/en-us/adminGuide-adminKitchenDiningRoomReference#configItemSortingPriority) setting to sort items on kitchen tickets.

- To sort by assigned seat numbers, you must enable the seat numbers modifier group with the Additional Modifier Groups setting, and assign the Seat Number modifier group to the menu item. If two items have the same seat number, they are sorted by Sort Order, if available. If no Sort Order is available, the items are sorted by how they were added to the order. For more information, see [Additional Modifier Groups](docs/en-us/adminGuide-adminUiOptionsReference#configAdditionalModifierGroups).
...
1. If [Modifier display mode](docs/en-us/adminGuide-adminUiOptionsReference#configModifierDisplayMode) is set to:
...
3. If no Sort Order is configured for modifiers and [Modifier ordering priority](docs/en-us/adminGuide-adminUiOptionsReference#configModifierOrderingPriority) is set to On, the modifiers are sorted by modifier group.
...
4. The modifiers are then ordered based on the [Modifier group sorting](docs/en-us/adminGuide-adminUiOptionsReference#configModifierGroupSorting) setting option:
...
The Toast platform allows you to have separate tickets for each item in an order, or to have an order ticket with all items in an order. For example, you have an order with two items, Panini and Burger that both route to the Hot prep station. If using individual item tickets, this results in two tickets, one with Panini and another with Burger. If not using individual item tickets, this results in a single ticket with two items, Panini and Burger. The Individual Item Tickets setting has separate configurations for both [printed](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsPrinted)and [KDS tickets](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsKDS). Individual item tickets can apply to printed prep station tickets, and to KDS prep station and expediter tickets, depending on your setup.



> **Note**
> 
> Individual tickets apply differently for items with multiple quantities. Depending on your Individual Item Tickets setting options, you can choose to specify whether items with multiple quantities are consolidated into a single item with a multiple quantity, or as separate items each with a quantity of one. For more information, see [Consolidate identical items](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenConsolidateIdenticalItems).


The Individual Item Tickets setting has two parts for [Print Settings](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsPrinted) and [KDS Settings](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsKDS).
...
> **Note**
> 
> Individual item tickets and overview tickets only apply to printed prep station tickets. Printed expediter tickets always include all items in a course for an order, when configured to [Send to Expediter](docs/en-us/adminGuide-adminKitchenDiningRoomReference#configSendToExpediter).
...
> **Note**
> 
> Items only appear on the expediter KDS device if the prep station they are assigned to is configured to [Send to Expediter](docs/en-us/adminGuide-adminKitchenDiningRoomReference#configSendToExpediter).
...
Choosing how to configure individual items for printed and KDS tickets can affect how identical items are consolidated. For more information, see [Consolidate identical items](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenConsolidateIdenticalItems)Consolidating identical items.
...
For more information about these options, see [Print settings](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsPrinted).
...
For more information about these options, see [KDS settings](docs/en-us/adminGuide-platformKitchenConfiguringTickets#platformKitchenIndividualItemTicketsKDS).
