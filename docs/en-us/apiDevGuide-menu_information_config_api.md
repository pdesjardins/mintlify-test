...
> **Important**
> 
> The menus API has replaced the configuration API as the
        preferred mechanism for retrieving menu data for a restaurant. It is
        described in the [Menus API overview](apiDevGuide-apiGettingMenuInformationFromTheMenusAPI) section. This
        section remains for customers who have not yet transitioned to using
        the menus API for menu data retrieval.
...
In a Toast `Menu`, each `MenuGroup` has a
      ...
      represent menu components in [MenuItems, MenuGroups, MenuOptionGroups, and Menus](apiDevGuide-menu_information_config_api.html#menu-component-data-objects).
...
The following example shows the JSON representations of a menu,
      ...
      resource return data in [Working with the Toast menu hierarchy](apiDevGuide-menu_information_config_api.html#api-working-with-menu-hierarchy).
...
The following example shows a curl command that places an order
      for `'Award Winning' Curly-Q Fries` with `Full Basket` modifier with the **curl** command and
...
The following example demonstrates a simple raw menu
          ...
          information, see [External identifiers](apiDevGuide-portalToastIdentifiers#apiExternalIdentifiers).
...
In this case we can see that there are two options in the
          ...
          course be retrieved as above with `GET``/menuItems/<em>\<guid\></em>`.
