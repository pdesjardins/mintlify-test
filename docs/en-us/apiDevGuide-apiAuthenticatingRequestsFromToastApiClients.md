...
Your Toast outbound API integration can verify the authenticity of the
  ...
  [a Toast
  outbound API integration](apiDevGuide-apiUsingToastOutboundApis), the Toast platform sends REST requests to
...
When you implement an integration with a Toast outbound API, you
    ...
    about static API keys, see [Using static API keys](apiDevGuide-apiAuthenticatingRequestsFromToastApiClients#apiUsingStaticApiKeys).
...
**Example 10.1. HTTP `Authorization` header field for a Toast platform
      API client request**
...
(1) The string value of the HTTP header Authorization
          field is the static API key that you supply to Toast when you
          implement your integration. For more information, see Using static API keys.
...
Your authentication implementation should use different static API
    ...
    operations in the production environment. For more information, see [Environments](apiDevGuide-apiEnvironments).
...
> **Note**
> 
> If your integration and the Toast API client configuration are not
      coordinated when you rotate static API keys, API requests for your
      integration will fail. Make sure you coordinate static API key rotation
      with the Toast integrations team.
