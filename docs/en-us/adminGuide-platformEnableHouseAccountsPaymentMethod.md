...
Your guests can use a house account as a payment method to pay for Toast orders. To allow your guests to pay using a house account, you need to turn on the **Take house account payments on a POS**setting in Toast Web.
...
On the **Payment Terminal** screen on the Toast POS device, checks that have been paid by a house account show the total check amount and the house account icon. For more information, see [House accounts](adminGuide-platformPwfTenders#platformPwfHouseAccountsInfo).
...
9. On the order screen, select the house account****on the order details panel.
