...
2. If a Size modifier group exists for the menu item (and it does not have a Display Order Priority number, see the [note](docs/en-us/adminGuide-adminUnderstandingModifierGroupDisplay#adminNoteAboutSizeModifierGroupsWithDisplayOrderingPriorityNumbers)below), it is sorted after modifier groups that have a Display Ordering Priority number and before modifier groups that do not have a Display Ordering Priority number:
