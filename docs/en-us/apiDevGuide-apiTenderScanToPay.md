...
The Toast tender API scan-to-pay feature allows the Toast platform
    ...
    room, without the need to look up a guest’s account.

You can use scan-to-pay in the following ways: 

- Scan a QR code: The guest presents a QR code that, once
          scanned, directs a Toast transaction to the guest’s account.


- Enter a guest's unique identifier: On the Toast POS app, enter
          the guest’s unique identifier to charge their transaction to their
          account.
...
Before you can use scan-to-pay, you must be a Toast integration
    ...
    scan-to-pay feature. To become an integration partner, see the [Integration partnership process](apiDevGuide-integrationDevProcess).
...
If you scan a QR code, or enter a guest's unique identifier, and
      ...
      about how to respond to certain errors, see [Error handling](apiDevGuide-apiTenderProviderIntegrationsOverview#apiTenderErrorHandling).
...
3. After you select your scan-to-pay payment type, your
            device’s camera or QR scanner is activated. Position the guest’s
...
4. After the QR code is scanned, the guest account has been
            ...
            information about payment workflows, see [Payment workflow overview](adminGuide-platformPwfOverview).
...
Guests can be given a unique identifier to use at the POS during
        ...
        payload, see [Transaction descriptions](apiDevGuide-apiTenderPmsIntegration#apiTenderTransactionDescriptionsPMS).

The following procedure describes how to process a transaction
        using a guest’s unique identifier.

**Procedure 10.17. To complete a transaction with a unique guest
          identifier**
...
3. Enter your guest’s unique identifier and select
            Use Code.
...
> **Note**
> 
> You may select Scan With Camera to
              continue the payment process by scanning the guest’s QR code if
              available.
...
4. After the guest's unique identifier is entered, the payment
            ...
            workflows, see [Payment workflow overview](adminGuide-platformPwfOverview).
