...
The Publish later tab lists your change sets and their [statuses](adminGuide-platformUnderstandingScheduledPublishingAndChangeSets#platformChangeSetStatuses). For change sets that have a status of `FAILED` or `PARTIAL
...
| `You are not permitted to access this
                resource` | The [owner](adminGuide-ownersAndPermissions) of the menu item was changed and the employee who created the change set does not have the 4. Restaurant Admin \> 4.5 Edit Full Menu permission to the location or location group that now owns the menu item. | 
