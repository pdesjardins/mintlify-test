...
You can request labor reporting data for a single day, week, or
      ...
      API, see [Analytics API rate limits](apiDevGuide-apiAnalyticsRateLimiting).
...
> You can include restaurant GUIDs for only one of these values.
        ...
        labor reporting data. They are contradictory values that cannot be
        used together.
...
(4) Include details about the requested labor reporting data in
...
(5) This request for labor reporting data is grouped by
            job.
...
(1) The GUID for the labor reporting data request, also called
            the reportRequestGuid. For more information about how
...
For an example that shows how to retrieve the labor reporting
        data, see [Retrieving the labor reporting data](apiDevGuide-apiAnalyticsLaborReportingDataRetrieveData).
