...
Stock webhook messages follow the [standard message data schema](docs/en-us/apiDevGuide-apiMessageDataSchema). When a message is published to your webhook endpoint for the `stock`event category, the `eventCategory` value is set to `stock` and the `eventType` is set to one of the following:

- [in_stock](docs/en-us/apiDevGuide-apiStockWebhook#apiInStock)


- [out_of_stock](docs/en-us/apiDevGuide-apiStockWebhook#apiOutOfStock)


- [low_quantity](docs/en-us/apiDevGuide-apiStockWebhook#apiLowQuantity)



A menu item's inventory can be changed in the [stock API](docs/en-us/apiDevGuide-apiUpdatingInventoryInformation), in the Toast app, and in Toast Web, and set to one of the following:
