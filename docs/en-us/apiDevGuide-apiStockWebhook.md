...
> Modifiers are supported by an underlying menu item, called the
    modifier item reference. In this section, the term *menu
    item* refers to both menu items and modifier item references, as
    the stock webhook reports on their status in the same way. For detailed
    information on modifier item references, see [Understanding a modifier item reference](adminGuide-adminPricingModifierOptions#adminUnderstandingAModifierOptionsItemReference).


Stock webhook messages follow the [standard message data schema](apiDevGuide-apiMessageDataSchema). When
...
- [in_stock](apiDevGuide-apiStockWebhook#apiInStock)


- [out_of_stock](apiDevGuide-apiStockWebhook#apiOutOfStock)


- [low_quantity](apiDevGuide-apiStockWebhook#apiLowQuantity)



A menu item's inventory can be changed in the [stock
  API](apiDevGuide-apiUpdatingInventoryInformation), in the Toast app, and in Toast Web, and set to one of the
  following:
...
| Value | Description | 
| --- | --- |
| `itemGuid` | A unique identifier for the menu item, assigned by
            the Toast platform.data type:
            stringformat:
            uuid | 
| `multiLocationId` | A consistent identifier that applies to all versions
            of a menu item that is shared across
            locations.Requests sent to Toast APIs can use a
            combination of the `multiLocationId` and
            `restaurantGuid` values, instead of the
            `itemGuid` value, to identify a menu item. Integration
            clients can extract the `multiLocationId` and
            `restaurantGuid` from stock webhook messages to use in
            subsequent requests to Toast APIs. This is the recommended method
            for identifying menu items in Toast API requests. See [Toast identifiers](apiDevGuide-portalToastIdentifiers) for
            more information.data type:
            string | 
| `restaurantGuid` | A unique identifier for the restaurant, assigned by
            the Toast platform.data type:
            stringformat:
            uuid | 
| `quantity` | Indicates the quantity of this menu item that is
            available. This value is included if the `status`
            attribute is set to `QUANTITY` and omitted if the
            status attribute is set to
            `IN_STOCK`.data type:
            numberformat:
            double | 
| `status` | `IN_STOCK` or
...
**Example 9.13. Payload for an in_stock event when a menu item is manually marked
      as In Stock**
...
**Example 9.14. Payload for an in_stock event when a menu item's quantity is
