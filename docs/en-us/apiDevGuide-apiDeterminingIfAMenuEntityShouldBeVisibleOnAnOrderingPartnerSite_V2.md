...
> **Important**
> 
> This section applies to partners using menus API V2 only. For
        partners using menus API V3, menu entities are pre-filtered to exclude
        menu entities that should not be visible on their sites. For more
        information, see [Comparing menus API V2 and V3](apiDevGuide-apiComparingMenusAPIV2AndV3).
...
The channel visibility settings reside on a menu entity's details
      ...
      menu details pages or the newer menu builder tool](adminGuide-adminBasicMenuBuilderAndTheLegacyMenuDetailsPages) to edit the
...
| --- | --- | --- | --- |
| In-store orders: POS | POS | `POS` | The menu entity is visible on a Toast POS
              device. | 
| In-store orders: Kiosk | Kiosk, Toast Order and Pay | `KIOSK` | The menu entity is visible on a Toast
              Kiosk. | 
| Online orders: Toast | Toast Online Ordering, Toast Takeout (Local by
              Toast) app | `TOAST_ONLINE_ORDERING` | The menu entity is visible on a restaurant's Toast
              online ordering site. | 
| Online orders: Ordering partners | Online ordering partners | `ORDERING_PARTNERS` | The restaurant wants the menu entity to be visible
              on all online ordering sites that they have allowed to integrate
